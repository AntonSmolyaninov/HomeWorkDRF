...
class PaymentFilter(filters.FilterSet):
    ...
    course__title = filters.CharFilter(field_name="course__title", lookup_expr="icontains")

    # Фильтр по уроку (по ID урока)
    lesson = filters.NumberFilter(field_name="lesson__id", lookup_expr="exact")
    lesson__title = filters.CharFilter(field_name="lesson__title", lookup_expr="icontains")

    # Фильтр по способу оплаты
    payment_method = filters.ChoiceFilter(field_name="payment_method", choices=Payment.PaymentMethod.choices)

    # Фильтр по дате (диапазон)
    payment_date_from = filters.DateTimeFilter(field_name="payment_date", lookup_expr="gte")
    payment_date_to = filters.DateTimeFilter(field_name="payment_date", lookup_expr="lte")
